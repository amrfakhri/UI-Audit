## Privacy Policy

We value your privacy and are committed to protecting your personal information. This Privacy Policy explains how we collect, use, and safeguard your data when you use our app.

### Information We Collect
We may collect the following types of information:
- **Personal information** (e.g. name, phone number, ID/Iqama number)
- **Account information** (e.g. login details, preferences)
- **Transaction data** (e.g. payment history, merchant details)
- **Device and usage data** (e.g. device type, app interactions)

### How We Use Your Information
We use your information to:
- Provide and improve our services
- Process transactions and scheduled payments
- Verify your identity and prevent fraud
- Communicate important updates or notifications
- Comply with legal and regulatory requirements

### Data Sharing
We do not sell your personal data. We may share your information only when necessary with:
- Trusted service providers (to operate the app)
- Regulatory authorities (if required by law)

### Data Security
We implement appropriate security measures to protect your data from unauthorized access, loss, or misuse.

### Data Retention
We retain your information only for as long as necessary to fulfill the purposes outlined in this policy or as required by law.

### Your Rights
Depending on your location, you may have the right to:
- Access your personal data
- Request correction or deletion
- Object to or restrict certain processing

### Changes to This Policy
We may update this Privacy Policy from time to time. Any changes will be reflected in this section.

### Contact Us
If you have any questions about this Privacy Policy, please contact us at:
[amr.fakhri@gmail.com]